...
class ResourceEvents extends Component {
  ...
  updateRealRowHeight = () => {
    ...
    const realHeight = this.eventContainer.scrollHeight;

    if (realHeight !== this.props.resourceEvents.rowHeight) {
      this.props.resourceEvents.rowHeight = realHeight;

      this.props.schedulerData._createRenderData();
      this.forceUpdate();
    }
  };
  ...
  stopDrag = (ev) => {
    ...
    if (cellUnit !== CellUnit.Hour) {
      endTime = localeDayjs(
        new Date(resourceEvents.headerItems[rightIndex - 1].start)
      )
        .hour(23)
        .minute(59)
        .second(59)
  ...
  onAddMoreClick = (headerItem) => {
    ...
    if (onSetAddMoreState) {
      ...
      if (index !== -1) {
        ...
        const resourceItem = renderData.find(
          (r) => r.slotId === resourceEvents.slotId
        );
        if (resourceItem) {
          resourceItem.rowHeight = resourceEvents.rowHeight;
        }
        onSetAddMoreState({
          headerItem,
          left,
  ...
  render() {
    ...
    const eventContainer = (
      <div
        ...
        style={{ minHeight: resourceEvents.rowHeight }}
      >
        {selectedArea}
        {eventList}
      </div>
